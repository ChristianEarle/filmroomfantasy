...
// .wrangler/tmp/bundle-VW9ecb/checked-fetch.js
...
// .wrangler/tmp/bundle-VW9ecb/strip-cf-connecting-ip-header.js
...
async function getPlayerStatsSummary(db, playerId, seasonYear) {
  ...
  const totals = stats.reduce((acc, week2) => {
    const played = (week2.offSnaps ?? 0) > 0 || (week2.defSnaps ?? 0) > 0 || (week2.stSnaps ?? 0) > 0 || ((week2.passAttempts ?? 0) > 0 || (week2.rushAttempts ?? 0) > 0 || (week2.targets ?? 0) > 0 || (week2.receptions ?? 0) > 0 || (week2.fgAttempts ?? 0) > 0 || (week2.xpAttempts ?? 0) > 0 || (week2.sacks ?? 0) > 0 || (week2.defInterceptions ?? 0) > 0);
    const snapPct = (() => {
...
function parseItemBlock(block, defaultAuthor) {
  ...
  const link = linkMatch ? (linkMatch[1] || "").replace(/<[^>]+>/g, "").trim() || null : null;
  const desc3 = contentMatch ? contentMatch[1] : descMatch ? descMatch[1] : "";
  const text2 = extractText(desc3) || title;
...
adminRoutes.post("/sync-news", async (c) => {
  ...
  try {
    const raw2 = await fetchSleeperPlayers();
    const INJURY_STATUSES = /* @__PURE__ */ new Set(["out", "doubtful", "questionable", "ir", "injured_reserve", "inactive"]);
    ...
    for (const [sleeperId, p] of Object.entries(raw2)) {
      ...
      if (EXCLUDED_STATUSES.has(injuryStatus) || EXCLUDED_STATUSES.has(status))
        continue;
      if ((injuryStatus === "inactive" || status === "inactive") && injuryNotes.length === 0)
        continue;
      const hasMeaningfulStatus = INJURY_STATUSES.has(injuryStatus) || INJURY_STATUSES.has(status) || injuryNotes.length > 0;
      ...
      const headline = injuryNotes.length > 0 ? `${name} - ${statusLabel}: ${injuryNotes}` : `${name} - ${statusLabel}`;
      const content = injuryNotes.length > 0 ? injuryNotes : `${name} is listed as ${statusLabel}.`;
...
adminRoutes.post("/sync-twitter-news", async (c) => {
  ...
  try {
    ...
    const twitterNews = await db.query.playerNews.findMany({
      where: eq(playerNews.source, "Twitter"),
      columns: { id: true }
    });
    for (const n of twitterNews) {
    ...
    for (const tweet of tweets) {
      ...
      const textStart = text2.slice(0, 180);
      for (const player of playersByNameLength) {
        ...
        if (seen.has(key))
          continue;
        seen.add(key);
        const headline = tweet.text.length > 120 ? tweet.text.slice(0, 117) + "..." : tweet.text;
        await db.insert(playerNews).values({
          id: generateId5(),
          playerId: player.id,
          headline,
...
// .wrangler/tmp/bundle-VW9ecb/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VW9ecb/middleware-loader.entry.ts
