...
// .wrangler/tmp/bundle-kIrW81/checked-fetch.js
...
// .wrangler/tmp/bundle-kIrW81/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-kIrW81/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-kIrW81/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
var projectionLineSnapshots = sqliteTable("projection_line_snapshots", {
  id: text("id").primaryKey(),
  playerId: text("player_id").notNull().references(() => nflPlayers.id, { onDelete: "cascade" }),
  week: integer("week").notNull(),
  seasonYear: integer("season_year").notNull(),
...
var gameLineSnapshots = sqliteTable("game_line_snapshots", {
  id: text("id").primaryKey(),
...
var userFeedback = sqliteTable("user_feedback", {
  id: text("id").primaryKey(),
  ...
  if (league.platform === "sleeper" && league.externalId) {
    try {
      ...
      const matchupResponses = await Promise.all(
        weekNumbers.map(
          (week2) => fetch(`https://api.sleeper.app/v1/league/${league.externalId}/matchups/${week2}`).then((res) => res.ok ? res.json() : null).catch(() => null)
        )
      );
      for (let i = 0; i < weekNumbers.length; i++) {
        const week2 = weekNumbers[i];
        const weekMatchups = matchupResponses[i];
        if (!weekMatchups)
          continue;
      ...
      try {
        ...
        if (projectionsResponse.ok) {
          ...
          const gamesForWeek = await db.query.nflGames.findMany({
            where: and(eq(nflGames.week, currentWeek), eq(nflGames.seasonYear, league.seasonYear)),
  ...
  try {
    ...
    if (week2 !== void 0 && week2 >= 1 && week2 <= 18) {
      ...
      if (!weekComplete && includeStats) {
        const statsCount = await db.select({ count: sql`count(*)` }).from(playerWeeklyStats).where(and(
          eq(playerWeeklyStats.week, week2),
          eq(playerWeeklyStats.seasonYear, season)
        ));
        const n = statsCount[0]?.count ?? 0;
        if (n >= 50)
          weekComplete = true;
      }
    }
    if (week2 !== void 0 && weekComplete && includeStats) {
      const stats = await db.query.playerWeeklyStats.findMany({
        where: and(
          eq(playerWeeklyStats.week, week2),
          eq(playerWeeklyStats.seasonYear, season)
      ...
      const CHUNK = 50;
      const idChunks = [];
      for (let i = 0; i < playerIdsFromStats.length; i += CHUNK)
        idChunks.push(playerIdsFromStats.slice(i, i + CHUNK));
      ...
      if (position && position !== "ALL" && position !== "FLEX")
        enriched = enriched.filter((p) => p.position === position);
      if (position === "FLEX")
        enriched = enriched.filter((p) => ["RB", "WR", "TE"].includes(p.position));
      if (team)
        enriched = enriched.filter((p) => p.team === team);
      if (search)
        enriched = enriched.filter((p) => p.name?.toLowerCase().includes(search?.toLowerCase()));
      if (availableOnly && leagueId)
        enriched = enriched.filter((p) => !p.isRostered);
...
playerRoutes.get("/projection-movements", optionalAuthMiddleware, async (c) => {
  ...
  try {
    ...
    for (const s of snapshots) {
      if (!earliestByPlayer.has(s.playerId))
        earliestByPlayer.set(s.playerId, { projectedPoints: s.projectedPoints, snapshotAt: s.snapshotAt });
    }
    ...
    if (gameComplete) {
      ...
      for (const s of stats) {
        ...
        if (statOpponent) {
          ...
          if (statOpponent !== expectedOpponent)
            continue;
        }
...
adminRoutes.post("/sync-stats", async (c) => {
  ...
  try {
    ...
    const seasonYear = body.seasonYear ?? 2025;
