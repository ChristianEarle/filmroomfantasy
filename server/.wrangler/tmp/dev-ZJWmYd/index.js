...
// .wrangler/tmp/bundle-V0PKtM/checked-fetch.js
...
// .wrangler/tmp/bundle-V0PKtM/strip-cf-connecting-ip-header.js
  ...
  if (league.platform === "sleeper" && league.externalId) {
    try {
      ...
      for (const roster of rosters) {
        ...
        const isUserTeam = userTeam && !userRosterAssigned && (userSleeperUserId ? roster.owner_id === userSleeperUserId : true);
        let team;
        if (isUserTeam) {
          team = userTeam;
...
playerRoutes.get("/", optionalAuthMiddleware, async (c) => {
  ...
  try {
    ...
    if (includeStats) {
      enrichedPlayers = await Promise.all(players.map(async (player) => {
        const stats = await db.query.playerWeeklyStats.findMany({
          where: and(
            eq(playerWeeklyStats.playerId, player.id),
  ...
  try {
    ...
    for (let week2 = 1; week2 <= maxWeeks; week2++) {
      try {
        ...
        const weekStats = await statsResponse.json();
        for (const sleeperPlayerId of Object.keys(weekStats)) {
          const playerStats = weekStats[sleeperPlayerId];
          if (!playerStats)
            continue;
          const player = await db.query.nflPlayers.findFirst({
...
// .wrangler/tmp/bundle-V0PKtM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-V0PKtM/middleware-loader.entry.ts
