...
// .wrangler/tmp/bundle-ZFUSp9/checked-fetch.js
...
// .wrangler/tmp/bundle-ZFUSp9/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-ZFUSp9/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
      ...
      for (const roster of rosters) {
        ...
        if (isUserTeam) {
          ...
          await db.update(teams).set({
            ...
            faabBudget: roster.settings?.waiver_budget_used != null ? Math.max(0, (league.waiverBudget || 100) - roster.settings.waiver_budget_used) : league.waiverBudget || 100,
            updatedAt: /* @__PURE__ */ new Date()
  ...
  try {
    ...
    if (teamIds.length > 0) {
      for (const tid of teamIds) {
        await db.delete(rosterSpots).where(eq(rosterSpots.teamId, tid));
      }
      for (const tid of teamIds) {
        await db.delete(matchups).where(eq(matchups.homeTeamId, tid));
        await db.delete(matchups).where(eq(matchups.awayTeamId, tid));
      }
      for (const tid of teamIds) {
        await db.delete(transactions).where(eq(transactions.addTeamId, tid));
        await db.delete(transactions).where(eq(transactions.dropTeamId, tid));
      }
...
playerRoutes.get("/trending", optionalAuthMiddleware, async (c) => {
  ...
  try {
    ...
    const sleeperTrending = await sleeperRes.json();
    ...
    let leagueRosteredIds = /* @__PURE__ */ new Set();
    ...
    if (leagueId) {
      ...
      if (leagueTeams.length > 0) {
        const teamIds = leagueTeams.map((t) => t.id);
        const rosterRows = await db.select({ playerId: rosterSpots.playerId }).from(rosterSpots).where(inArray(rosterSpots.teamId, teamIds));
        leagueRosteredIds = new Set(rosterRows.map((r) => r.playerId));
      }
    }
    const internalIds = allPlayers.map((p) => p.id);
    const avgPointsMap = /* @__PURE__ */ new Map();
    if (internalIds.length > 0) {
      for (let i = 0; i < internalIds.length; i += CHUNK_SIZE) {
        ...
        const statsRows = await db.select({
          ...
          avgPts: sql`ROUND(AVG(${playerWeeklyStats.fantasyPointsPPR}), 1)`.as("avg_pts")
        }).from(playerWeeklyStats).where(inArray(playerWeeklyStats.playerId, chunk)).groupBy(playerWeeklyStats.playerId);
        for (const row of statsRows) {
          avgPointsMap.set(row.playerId, row.avgPts ?? 0);
        }
      }
    }
    const trending = sleeperTrending.map((t) => {
      ...
      return {
        ...
        trendValue: t.count,
        ownedPct,
        ownedInLeague: leagueId ? leagueRosteredIds.has(player.id) : void 0,
...
var publicRateLimit = rateLimit(60, 60 * 1e3);
var espnProxyRateLimit = rateLimit(20, 60 * 1e3);
...
var feedbackRateLimit = rateLimit(5, 15 * 60 * 1e3);
