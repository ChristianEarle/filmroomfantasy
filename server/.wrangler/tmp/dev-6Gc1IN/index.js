...
// .wrangler/tmp/bundle-NMOzSV/checked-fetch.js
...
// .wrangler/tmp/bundle-NMOzSV/strip-cf-connecting-ip-header.js
  ...
  if (league.platform === "sleeper" && league.externalId) {
    try {
      ...
      for (const roster of rosters) {
        ...
        const isUserTeam = userTeam && !userRosterAssigned;
        let team;
        if (isUserTeam) {
          team = userTeam;
          ...
          if (existingTeam) {
            ...
            await db.insert(teams).values({
              ...
              leagueId: league.id,
              ownerId: user.id,
              // Placeholder - all teams owned by syncing user for now
              name: teamName,
              wins: roster.settings?.wins || 0,
              losses: roster.settings?.losses || 0,
              ties: roster.settings?.ties || 0,
      ...
      return c.json({
        success: true,
        message: `League synced successfully from Sleeper. ${rosters.length} teams updated.`,
        teamsUpdated: rosters.length
      });
    } catch (error) {
      console.error("Sleeper sync error:", error);
      return c.json({ error: "Failed to sync league from Sleeper" }, 500);
    }
  }
  return c.json({
    success: true,
...
playerRoutes.get("/", optionalAuthMiddleware, async (c) => {
  ...
  try {
    ...
    const players = await db.query.nflPlayers.findMany({
      ...
      limit,
      offset,
      orderBy: sortOrder === "desc" ? desc(nflPlayers[sortBy] || nflPlayers.name) : asc(nflPlayers[sortBy] || nflPlayers.name)
    });
...
waiverRoutes.delete("/:claimId", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (!claim.addTeam || claim.addTeam.ownerId !== user.id) {
...
// .wrangler/tmp/bundle-NMOzSV/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-NMOzSV/middleware-loader.entry.ts
