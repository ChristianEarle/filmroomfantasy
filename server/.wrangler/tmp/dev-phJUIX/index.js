...
// .wrangler/tmp/bundle-1YLwum/checked-fetch.js
...
// .wrangler/tmp/bundle-1YLwum/strip-cf-connecting-ip-header.js
...
var init_nfl_schedule_2025 = __esm({
  "src/data/nfl-schedule-2025.json"() {
    nfl_schedule_2025_default = {
      weeks: {
        "1": [
          { away: "DAL", home: "PHI", date: "2025-09-04T20:20:00-04:00", network: "NBC", stadium: "Lincoln Financial Field" },
          { away: "KC", home: "LAC", date: "2025-09-05T20:00:00-04:00", network: "YouTube", stadium: "Corinthians Arena (S\xE3o Paulo)" },
          { away: "TB", home: "ATL", date: "2025-09-07T13:00:00-04:00", network: "FOX", stadium: "Mercedes-Benz Stadium" },
          { away: "CIN", home: "CLE", date: "2025-09-07T13:00:00-04:00", network: "CBS", stadium: "Cleveland Browns Stadium" },
          { away: "WAS", home: "NYG", date: "2025-09-07T13:00:00-04:00", network: "FOX", stadium: "MetLife Stadium" },
          { away: "DEN", home: "TEN", date: "2025-09-07T16:00:00-04:00", network: "FOX", stadium: "Nissan Stadium" },
          { away: "SF", home: "SEA", date: "2025-09-07T16:00:00-04:00", network: "FOX", stadium: "Lumen Field" },
          { away: "DET", home: "GB", date: "2025-09-07T16:00:00-04:00", network: "FOX", stadium: "Lambeau Field" },
          { away: "LAR", home: "HOU", date: "2025-09-07T16:00:00-04:00", network: "FOX", stadium: "NRG Stadium" },
          { away: "BAL", home: "BUF", date: "2025-09-07T20:20:00-04:00", network: "NBC", stadium: "Highmark Stadium" },
          { away: "MIN", home: "CHI", date: "2025-09-08T20:15:00-04:00", network: "ESPN", stadium: "Soldier Field" }
        ],
        "2": [
          { away: "IND", home: "GB", date: "2025-09-11T20:15:00-04:00", network: "ESPN", stadium: "Lambeau Field" },
          { away: "ATL", home: "PHI", date: "2025-09-14T13:00:00-04:00", network: "FOX", stadium: "Lincoln Financial Field" },
          { away: "LAC", home: "CAR", date: "2025-09-14T13:00:00-04:00", network: "CBS", stadium: "Bank of America Stadium" },
          { away: "NYJ", home: "TEN", date: "2025-09-14T13:00:00-04:00", network: "CBS", stadium: "Nissan Stadium" },
          { away: "SEA", home: "NE", date: "2025-09-14T13:00:00-04:00", network: "FOX", stadium: "Gillette Stadium" },
          { away: "NO", home: "DAL", date: "2025-09-14T13:00:00-04:00", network: "FOX", stadium: "AT&T Stadium" },
          { away: "LV", home: "BAL", date: "2025-09-14T13:00:00-04:00", network: "CBS", stadium: "M&T Bank Stadium" },
          { away: "CLE", home: "JAX", date: "2025-09-14T13:00:00-04:00", network: "CBS", stadium: "EverBank Stadium" },
          { away: "CHI", home: "HOU", date: "2025-09-14T13:00:00-04:00", network: "FOX", stadium: "NRG Stadium" },
          { away: "MIA", home: "BUF", date: "2025-09-14T13:00:00-04:00", network: "CBS", stadium: "Highmark Stadium" },
          { away: "LAR", home: "ARI", date: "2025-09-14T16:05:00-04:00", network: "FOX", stadium: "State Farm Stadium" },
          { away: "TB", home: "DET", date: "2025-09-14T16:05:00-04:00", network: "FOX", stadium: "Ford Field" },
          { away: "KC", home: "CIN", date: "2025-09-14T16:25:00-04:00", network: "CBS", stadium: "Paycor Stadium" },
          { away: "NE", home: "SEA", date: "2025-09-14T16:25:00-04:00", network: "CBS", stadium: "Lumen Field" },
          { away: "DEN", home: "TB", date: "2025-09-14T20:20:00-04:00", network: "NBC", stadium: "Raymond James Stadium" },
          { away: "PIT", home: "DEN", date: "2025-09-15T20:15:00-04:00", network: "ESPN", stadium: "Empower Field at Mile High" }
        ]
      }
    };
  }
...
function loadStaticSchedule(week2, season, seasonType) {
  ...
  weekGames.forEach((g, i) => {
    ...
    games.push({
      ...
      overUnder: null,
      tvNetwork: network,
      weather: g.stadium?.toLowerCase().includes("indoor") ? { displayValue: "Indoor", temperature: 72 } : null
    });
    dbRows.push({
      id,
      week: week2,
...
async function fetchEspnScoreboard(week2, season, seasonType) {
  ...
  try {
    const params = new URLSearchParams({ season: String(s), seasontype: st });
    if (week2 != null)
      params.set("week", String(week2));
    ...
    if (!res.ok) {
      ...
      throw new Error(`ESPN API ${res.status}: ${body.slice(0, 200) || res.statusText}`);
    }
    let data;
    try {
      data = await res.json();
    ...
    if (!data || typeof data !== "object") {
      throw new Error("ESPN returned unexpected format");
    }
    const events = data.events || [];
    const weekInfo = data.week;
    const resolvedWeek = week2 ?? weekInfo?.number ?? 1;
    const games = [];
    const dbRows = [];
    for (const ev of events) {
      ...
      const spread = spreadVal ? Math.abs(parseFloat(String(spreadVal).replace(/[+-]/g, "")) || 0) : null;
      const overUnder = overUnderVal ? parseFloat(String(overUnderVal).replace(/[ou]/gi, "")) || null : null;
      const weatherObj = weather ? { displayValue: weather.displayValue, temperature: weather.temperature } : venue?.indoor ? { displayValue: "Indoor", temperature: 72 } : null;
      const gameTime = new Date(ev.date);
      const gameTimeDisplay = ev.status?.type?.detail ?? ev.shortName ?? gameTime.toLocaleString("en-US", {
        weekday: "short",
        hour: "numeric",
        minute: "2-digit",
        timeZoneName: "short"
      ...
      games.push({
        ...
        overUnder,
        tvNetwork: Array.isArray(broadcast) ? broadcast[0] : broadcast || "TBD",
        weather: weatherObj
      });
      dbRows.push({
        id: ev.id,
        ...
        spread,
        overUnder,
        tvNetwork: Array.isArray(broadcast) ? broadcast[0] : broadcast || null,
    ...
    return { games, dbRows, week: resolvedWeek, season: s, source: "espn" };
  } catch (err) {
    if (s === 2025 && st === "2") {
      console.warn("ESPN API failed, using static 2025 schedule:", err instanceof Error ? err.message : err);
...
var ESPN_SCOREBOARD, TEAM_NAMES;
var init_espn = __esm({
  "src/services/espn.ts"() {
    "use strict";
    init_checked_fetch();
...
// .wrangler/tmp/bundle-1YLwum/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-1YLwum/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
authRoutes.put("/profile", authMiddleware, async (c) => {
  try {
    const user = c.get("user");
    const body = await c.req.json();
    const { username, avatarUrl, preferredScoring, darkMode, notificationsEnabled } = body;
    if (!user) {
      return c.json({ error: "Not authenticated" }, 401);
    }
    const db = c.get("db");
    ...
    if (preferredScoring !== void 0)
      updates.preferredScoring = preferredScoring;
    if (darkMode !== void 0)
...
function mapStatus(sleeperStatus, injuryStatus) {
  ...
  if (status === "inactive")
    return "inactive";
  if (status === "invalid" || injury === "invalid")
    return "active";
  return "active";
}
...
async function getPlayerStatsSummary(db, playerId, seasonYear, position) {
  ...
  const totals = stats.reduce((acc, week2) => {
    ...
    const snapPct = (() => {
      ...
      if (off > 0 && tmOff > 0)
        return off / tmOff * 100;
      if (def > 0 && tmDef > 0)
        return def / tmDef * 100;
      if (st > 0 && tmSt > 0)
        return st / tmSt * 100;
      return null;
    })();
  ...
  try {
    ...
    if (includeStats && players.length > 0) {
      ...
      enrichedPlayers = players.map((player) => {
        ...
        const projPts = projection?.projectedPoints || 0;
...
playerRoutes.get("/:id/stats", optionalAuthMiddleware, async (c) => {
  ...
  try {
    ...
    const computeSnapPct = /* @__PURE__ */ __name((row) => {
      ...
      if (off > 0 && tmOff > 0)
        return Math.round(off / tmOff * 1e3) / 10;
      if (def > 0 && tmDef > 0)
        return Math.round(def / tmDef * 1e3) / 10;
      if (st > 0 && tmSt > 0)
        return Math.round(st / tmSt * 1e3) / 10;
      return null;
    }, "computeSnapPct");
...
gameRoutes.get("/slate", optionalAuthMiddleware, async (c) => {
  ...
  const ctx = getNflSeasonContext();
  const season = 2025;
  const seasontype = week2 != null && week2 >= 1 && week2 <= 18 ? "2" : ctx.seasontype;
  try {
    let dbGames = [];
    if (week2 != null) {
    ...
    if (dbGames.length > 0) {
      const games2 = dbGames.map((g) => {
        const weather = g.weather ? JSON.parse(g.weather) : null;
      ...
      return c.json({
        week: dbGames[0]?.week ?? week2,
        season,
        weekLabel: week2 != null ? `Week ${week2}` : "",
        games: games2
      });
    }
    let games;
    ...
    let s;
    try {
      const result = await fetchEspnScoreboard(week2, season, seasontype);
      games = result.games;
      w = result.week;
      s = result.season;
      await persistGamesToDb(db, result.dbRows);
    } catch (espnError) {
      const fallbackWeek = week2 ?? 15;
      const dbFallback = await db.query.nflGames.findMany({
        where: and(
          eq(nflGames.week, fallbackWeek),
          eq(nflGames.seasonYear, season)
        ),
        orderBy: asc(nflGames.gameTime)
      });
      if (dbFallback.length > 0) {
        games = dbFallback.map((g) => {
          const weather = g.weather ? JSON.parse(g.weather) : null;
          const gameTime = new Date(g.gameTime);
          return {
            id: g.id,
        ...
        w = fallbackWeek;
        s = season;
        console.warn("ESPN failed, using DB fallback:", espnError instanceof Error ? espnError.message : espnError);
      } else {
        return c.json({
          week: week2 ?? 15,
          season,
          weekLabel: `Week ${week2 ?? 15}`,
          games: [],
    ...
    return c.json({
      week: w,
      season: s,
      weekLabel: week2 != null ? `Week ${week2}` : `Week ${w}`,
      games
    });
  } catch (error) {
...
function extractUrl(block, desc2) {
  ...
  if (linkMatch)
    return clean(linkMatch[1]);
  const atomLink = block.match(/<link[^>]+href=["']([^"']+)["'][^>]*>/i);
  if (atomLink)
    return clean(atomLink[1]);
...
Rules: relevantPlayerNames must be a subset of the listed players. summary must be 1-2 sentences. If no players are relevant, use empty array [].`;
  try {
    ...
    console.error("AI relevance check error:", e);
    return null;
  }
}
...
adminRoutes.post("/sync-news", async (c) => {
  ...
  try {
    ...
    for (const [sleeperId, p] of Object.entries(raw2)) {
      ...
      if (EXCLUDED_STATUSES.has(injuryStatus) || EXCLUDED_STATUSES.has(status))
        continue;
...
adminRoutes.post("/sync-twitter-news", async (c) => {
  ...
  try {
    ...
    const rssNews = await db.query.playerNews.findMany({
      where: ne(playerNews.source, "Sleeper"),
      columns: { id: true }
    });
    for (const n of rssNews) {
      await db.delete(playerNews).where(eq(playerNews.id, n.id));
    }
    let inserted = 0;
    const seen = /* @__PURE__ */ new Set();
    ...
    for (const tweet of tweets) {
      if (tweet.text.length > MAX_ITEM_LENGTH)
        continue;
      if (!tweet.url || !tweet.url.trim())
        continue;
      ...
      for (const player of playersByNameLength) {
        ...
        if (!name || name.length < 4)
          continue;
        if (!text2.includes(name.toLowerCase()))
          continue;
        if (!textStart.includes(name.toLowerCase()))
          continue;
...
app.use("*", cors({
  origin: (origin) => {
    ...
    if (allowedOrigins.includes(origin))
      return origin;
    if (origin.endsWith(".pages.dev") || origin.endsWith(".cloudflarepages.com"))
      return origin;
    return allowedOrigins[0];
  },
