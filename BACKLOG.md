...
### Player Card
- [ ] **Bring back quick actions on player cards** - Re-add the Quick Action buttons (Alert, Pin, Compare, Share) with real functionality. Previously removed because they were non-functional stubs. Watch, Share, and Compare are done (Compare via `useCompare` + `PlayerComparisonModal`, floating `CompareBar`); Alert and Pin remain.
...
### Standalone Pages & AI Features
> Major feature additions discussed during the player-card-link verification session. Today the app is modal-based for player details and has no per-player URL. These items add real pages and weave AI throughout the product.
...
- [ ] **AI post-game analysis** - After each NFL game finalizes, generate a short AI recap focused on fantasy-relevant takeaways: who exceeded/missed expectations, target share / carry distribution shifts, injury-driven role changes, and waiver-wire implications. Surfaced on GameDetailModal and on the user's matchup recap. Triggered by a cron that watches for `gameStatus === 'final'` transitions. Caches per `(gameId, season, week)`.
