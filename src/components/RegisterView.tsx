...
export function RegisterView({ onRegister, onSwitchToLogin, isDarkMode = false }: RegisterViewProps) {
    ...
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950' : 'bg-slate-100'}`}>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 px-6 py-12">
        <div className="max-w-md mx-auto text-center">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Join FilmRoom</h1>
          <p className="text-blue-100">Start dominating your fantasy league today</p>
        </div>
      </div>

      {/* Register Form */}
      <div className="px-6 -mt-6">
        <div className={`max-w-md mx-auto rounded-xl border shadow-lg ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
          <div className={`p-6 border-b ${isDarkMode ? 'border-slate-700' : 'border-slate-200'}`}>
            <h2 className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Create your account</h2>
            <p className={`text-sm mt-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Get started with your free account</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:border-blue-500 transition-colors ${
                    isDarkMode
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-12 py-2.5 border rounded-lg text-sm focus:outline-none focus:border-blue-500 transition-colors ${
                    isDarkMode
                      ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500'
                      : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400'
                  }`}
            ...
            <button
              ...
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-600/50 disabled:cursor-not-allowed text-white rounded-lg font-semibold transition-colors flex items-center justify-center gap-2 group"
            >
              {isLoading ? (
          ...
          {/* Sign In Link */}
          <div className={`p-4 border-t text-center ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-200 bg-slate-50'}`}>
      ...
      <div className="text-center py-8">
        <p className={`text-sm ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
          Dominate your league with data-driven insights
        </p>
      </div>
    </div>
  );
...
function PasswordRequirement({ met, text, isDarkMode }: { met: boolean; text: string; isDarkMode: boolean }) {
  return (
    <div className={`flex items-center gap-2 text-xs ${met ? 'text-green-500' : isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
