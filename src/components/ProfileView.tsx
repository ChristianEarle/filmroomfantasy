...
export function ProfileView({ isDarkMode = true, onLogout }: ProfileViewProps) {
  ...
  const inputWrap = `flex h-12 w-full items-center gap-3 rounded-lg border text-sm transition-all focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent ${
    isDarkMode ? 'border-[#222] bg-[#1a1a1a]/50' : 'border-slate-200 bg-slate-50'
  }`;
  ...
  return (
    <div className="max-w-sm mx-auto space-y-16">
      ...
      <div>
        ...
        <p className={isDarkMode ? 'text-[#737373]' : 'text-[#555]'}>Manage your account and password</p>
      </div>
      ...
      <div className={`border rounded-lg overflow-hidden ${isDarkMode ? 'bg-[#111] border-[#222]' : 'bg-white border-slate-200'}`}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className={`text-sm font-medium ${isDarkMode ? 'text-[#737373]' : 'text-[#555]'}`}>Account info</h2>
            {!editingAccount && (
              <button
                onClick={handleStartEditing}
                className={`flex items-center gap-1.5 text-sm font-medium transition-colors ${isDarkMode ? 'text-[#737373] hover:text-slate-200' : 'text-[#555] hover:text-slate-700'}`}
              >
          ...
          <div className="space-y-4">
            <div>
              ...
              {editingAccount ? (
                <div className={inputWrap}>
                  <div className={`flex h-full w-11 shrink-0 items-center justify-center rounded-l-[7px] ${isDarkMode ? 'text-[#555]' : 'text-[#737373]'}`}>
                ...
                >
                  ...
                  <span className={`min-w-0 flex-1 py-0 pr-4 text-sm ${isDarkMode ? 'text-[#a3a3a3]' : 'text-slate-600'}`}>
                    {user?.email || '—'}
                  </span>
                </div>
              )}
            ...
            <div>
              <label className={`block text-sm font-medium mb-2 ${isDarkMode ? 'text-[#a3a3a3]' : 'text-slate-700'}`}>Username</label>
              {editingAccount ? (
                <div className={inputWrap}>
                  <div className={`flex h-full w-11 shrink-0 items-center justify-center rounded-l-[7px] ${isDarkMode ? 'text-[#555]' : 'text-[#737373]'}`}>
                    <span className={`text-base font-medium ${isDarkMode ? 'text-[#555]' : 'text-[#737373]'}`}>@</span>
                  </div>
                  <input
                    type="text"
                    value={editUsername}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className={`flex h-10 items-center justify-center rounded-lg border px-4 text-sm font-medium transition-colors ${
                    isDarkMode
                      ? 'border-slate-600 text-[#a3a3a3] hover:bg-[#1a1a1a]'
            ...
            {user?.hasGoogle && (
              <div className={`flex items-center gap-2.5 rounded-lg border p-3 ${isDarkMode ? 'border-[#222] bg-[#1a1a1a]/50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <span className={`text-sm ${isDarkMode ? 'text-[#a3a3a3]' : 'text-slate-600'}`}>Google account linked</span>
      ...
      <div className={`border rounded-lg overflow-hidden ${isDarkMode ? 'bg-[#111] border-[#222]' : 'bg-white border-slate-200'}`}>
        <div className={`p-6 border-b ${isDarkMode ? 'border-[#222]' : 'border-slate-200'}`}>
          <div className="flex items-center gap-3 mb-6">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isDarkMode ? 'bg-[#1a1a1a]' : 'bg-slate-100'}`}>
              <Lock className={`w-5 h-5 ${isDarkMode ? 'text-[#737373]' : 'text-[#555]'}`} />
            </div>
            <div>
          ...
          <form onSubmit={handleSavePassword} className="space-y-6">
            ...
            {!isGoogleOnly && (
              ...
              >
                ...
                <button
                  ...
                  className={`flex h-full w-10 shrink-0 items-center justify-center rounded-r-[7px] ${isDarkMode ? 'text-[#555] hover:text-[#a3a3a3]' : 'text-[#737373] hover:text-slate-600'} transition-colors`}
                >
            ...
            {newPassword.length > 0 && (
              <div className="space-y-2">
                <p className={`text-xs font-medium mb-1.5 ${isDarkMode ? 'text-[#737373]' : 'text-[#555]'}`}>New password must include:</p>
...
function PasswordRequirement({ met, text, isDarkMode }: { met: boolean; text: string; isDarkMode: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 text-xs ${met ? 'text-green-500' : isDarkMode ? 'text-[#555]' : 'text-[#737373]'}`}>
      <div className={`flex h-4 w-4 shrink-0 items-center justify-center rounded ${met ? 'bg-green-500/20 border border-green-500/30' : isDarkMode ? 'border border-[#222] bg-[#1a1a1a]' : 'border border-slate-200 bg-slate-100'}`}>
