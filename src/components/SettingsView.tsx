...
export function SettingsView({ isDarkMode = true, onToggleDarkMode, onLeagueSynced }: SettingsViewProps) {
  ...
  const handleYahooConnect = async () => {
    ...
    try {
      ...
      // A 503 means the server has no Yahoo OAuth secrets — retrying can never
      // work, so surface the server's explanation and disable the tile instead
      // of telling the user to try again.
      if (err instanceof ApiError && err.status === 503) {
        setYahooConfigured(false);
        setYahooError(err.message);
      } else {
        setYahooError(
          err instanceof ApiError && err.message
            ? err.message
            : 'Failed to start Yahoo authorization. Please try again.'
        );
      }
      setConnectionStep('select-platform');
    }
            ...
            <div className="p-6">
              ...
              {connectionStep === 'select-platform' && (
                <div className="space-y-4">
                  ...
                  {/* Yahoo failures reset the flow to this step, so the error
                      has to render here or it's set and never seen. */}
                  {yahooError && (
                    <div className="flex items-center gap-2 p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-500 text-sm">
                      <AlertCircle className="w-4 h-4 flex-shrink-0" aria-hidden="true" />
                      {yahooError}
