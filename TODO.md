...
- [x] **ROS rankings** — `GET /market-rankings` now takes a `sort=ros`
  param that re-ranks the same Market rows by remaining-season points
  instead of full-season VORP (nulls last); Draft Rankings' Market view
  gained a Season/ROS sort pill next to the source toggle.
