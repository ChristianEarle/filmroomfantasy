...
> **Status (post-cleanup sprint):** The cleanup sprint ripped out `Math.sin`-derived fake data columns. Five buttons are wired (Export, Compare, Add to compare, Trade value, Watch); **Ask AI** is the only remaining no-op button. The watchlist backend needs its migration applied to remote D1 + a worker deploy. Do Ask AI next, then the data-infrastructure items.
