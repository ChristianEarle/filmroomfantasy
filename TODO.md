...
- [x] **ROS rankings** — Market projections already carried a
  `rosProjectedPoints` number per player (#309) via `GET /market-rankings`;
  `DraftRankingsView`'s Market board now has a Season/ROS sort toggle that
  re-ranks the already-fetched board by remaining-season value client-side
  (no new endpoint or schema change needed).
