...
- [ ] **"Ask AI about draft" button** (~1–1.5 days) — Quick chat modal against the loaded rankings ("who should I draft at pick 5?"). Reuses the `POST /api/trades/follow-up` machinery (`server/src/routes/trades.ts:456-589`). Three commits:
...
- [ ] **Watch / watchlist** (~1–1.5 days) — Per-row "Watch" persists to a new table; nothing exists today. Two commits:
