...
- [x] Weather for outdoor games (Open-Meteo, free) — pre-game forecasts for
  upcoming outdoor games; ESPN's `weather` field only reports live/final
  conditions. See `fetchOutdoorForecast` in `server/src/services/espn.ts`.
