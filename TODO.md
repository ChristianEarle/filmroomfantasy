...
> **Status (post-cleanup sprint):** The cleanup sprint ripped out `Math.sin`-derived fake data columns. The four pure-frontend buttons are now wired (Export, Compare, Add to compare, Trade value); the two backend-dependent buttons (Ask AI, Watch) are still rendered and clickable but no-op until their backend lands. Do those two next, then the data-infrastructure items.
...
- [ ] **"Ask AI about draft" button** (~1 day) — Quick chat modal against the currently loaded rankings ("who should I draft at pick 5?"). Backend: new `POST /api/draft-rankings/ask` in `server/src/routes/draftRankings.ts` modeled on `POST /api/trades/follow-up` (`{ conversationHistory, question }` → `{ answer }`, `authMiddleware`, Pro/Elite gated via `TIER_REQUIRED`, reuse its rate-limit + daily-cap). Frontend: extract the `FollowUpChat` UI from `TradeAnalyzerView` into a shared `AiChatModal`, open from the Ask AI button, gate free tier with `UpgradeModal`.
- [ ] **Watch / watchlist** (~1–1.5 days frontend + backend) — Per-row "Watch" button persists to a new `user_player_watchlist(userId, playerId, addedAt)` table (migration `0036_*`). Routes `GET/POST/DELETE /api/watchlist` under `authMiddleware` (scope every query to the JWT `userId` to prevent IDOR), registered in `server/src/index.ts`. Frontend `useWatchlist()` hook, saved indicator, and a "Watching only" filter chip. Requires login.
